...
void Region::init(shared_ptr<ID3D11Device> device, int x, int z, unsigned int worldWidthIn, unsigned int regionWidthIn, string workingPathIn) {
...
HRESULT Region::loadTerrain() {
	...
	vector<VertexPositionColor> terrainVertices;
	...
	if (file.is_open()) {
		...
		for (int index = 0; index < regionSize / 2; index++) {
			...
			if (x < regionWidth && z < regionWidth) {
				// push indices
				if (pythag(1, terrainVertices[index].position.y - terrainVertices[index + (regionWidth + 1) + 1].position.y, 1) < pythag(1, terrainVertices[index + (regionWidth + 1)].position.y - terrainVertices[index + 1].position.y, 1)) {
					// bottom triangle
	...
	vertexBufferDesc.ByteWidth = sizeof(VertexPositionColor) * terrainVertices.size();
	...
	result = m_device->CreateBuffer(&vertexBufferDesc, &vertexData, &terrainVB);
	if (FAILED(result)) {
		return result;
	}
	...
	result = m_device->CreateBuffer(&indexBufferDesc, &indexData, &terrainIB);
	if (FAILED(result))
	{
		return false;
	}
