...
namespace Component {
	...
	//}
	shared_ptr<Component> Player::GetComponent(const unsigned int & id)
	{
		// Query in-memory list
		for (Player & player : GetComponents()) {
			if (player.ID == id) {
				return std::make_shared<Component>(player);
			}
		}
		// Not found
		return nullptr;
	}
	void Player::SaveAll(string directory)
	{
		for (Player & player : GetComponents()) {
			player.Save(directory);
		}
	}
	void Player::Attach(shared_ptr<Component> & component)
	{
		GetComponents().push_back(*dynamic_cast<Player*>(component.get()));
	}
	string Player::GetName()
	{
		return "Player";
	}
	shared_ptr<Component> Player::Create(std::ifstream & ifs)
	{
		return make_shared<Component>(Player());
	}
	vector<Player>& Player::GetComponents()
	{
		static vector<Player> components;
		return components;
	}
	//void Player::render() {
	//	XMFLOAT3 up, lookAt;
