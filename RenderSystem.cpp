...
{
	shared_ptr<Component::Position> position = m_player->GetComponent<Component::Position>("Position");
	float y = sinf(position->Rot.y);
	float r = cosf(position->Rot.y);
	float z = r*cosf(position->Rot.x);
	float x = r*sinf(position->Rot.x);

	XMVECTOR lookAt = Vector3(position->Pos) + Vector3(x, y, z);

	XMMATRIX view = XMMatrixLookAtRH(Vector3(position->Pos), lookAt, Vector3(0.f, 1.f, 0.f));
	return view;
}

void RenderSystem::RenderVBO(shared_ptr<Component::VBO> vbo, Microsoft::WRL::ComPtr<ID3D11DeviceContext> deviceContext, PrimitiveBatch<DirectX::VertexPositionColor>* batch)
{
