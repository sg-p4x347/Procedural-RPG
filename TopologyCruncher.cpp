...
void TopologyCruncher::Strip(vector<Vector3>& path, std::function<float(float&t)> roll, std::function<float(float&t)> width, float segmentLength)
{
	//----------------------------------------------------------------
	// Bezier curve and its derivative: https://pages.mtu.edu/~shene/COURSES/cs3621/NOTES/spline/Bezier/bezier-der.html

	static const int divisions = 60;
